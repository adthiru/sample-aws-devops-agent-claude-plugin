...
6. On `COMPLETED`: pull the consolidated summary (`--order DESC --max-items 10`), then trigger mitigation (2-5 min): `aws___call_aws(cli_command="aws devops-agent update-backlog-task --agent-space-id SPACE_ID --task-id TASK_ID --task-status PENDING_START --region us-east-1")`. Poll `get-backlog-task` until `COMPLETED` again. Then call `aws___call_aws(cli_command="aws devops-agent list-executions --agent-space-id SPACE_ID --task-id TASK_ID --region us-east-1")` to find the newest execution_id, and `aws___call_aws(cli_command="aws devops-agent list-journal-records --agent-space-id SPACE_ID --execution-id EXEC_ID --record-type mitigation_summary_md --region us-east-1")` to get the mitigation plan. Show the user the proposed fix; **do not** auto-apply IaC or commands.
